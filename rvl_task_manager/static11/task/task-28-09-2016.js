					...
					html +='<tr class="gradeA odd">'
							...
							+'<td width="15%" class=" ">'
								...
								+'<div class="" style="float:left;">'
									...
									+'Claim</a>'
								+'</a>'
								+'&nbsp&nbsp&nbsp&nbsp&nbsp<label class ="history" onclick="getHistory('+"'"+loadData[i].processInstanceId+"','"+App_Mem_ID+"'"+');" ><i class="entypo-back-in-time" style="font-size:medium;"></i></label>'
								+'</div>'
							    +'</div>'
							+'</td>'	
						+'</tr>';
							...
							if(data2["mlValidationArray"]){
											    ...
											    if(historyDict[j]["activityName"] == remarksDict[i]["taskName"] && remarksDict[i]["task_id"] == historyDict[j]["taskId"] ){
										     		historyDict[j]["remarks"] = remarksDict[i]["remarks"];
											    	    if(remarksDict[i]["remarks"] && remarksDict[i]["validation_last_modified_date"]){
															historyDict[j]["remarks"] =remarksDict[i]["remarks"];
															historyDict[j]["remarksDate"]= 'Commented by <label style="font-size:11px;color:sienna;font-weight:bold;">'+remarksDict[i]["commented_user"].toUpperCase()+ "</label> on " +remarksDict[i]["validation_last_modified_date"];
										   				 }
														else{
								...
								for(var i=0;i<historyDict.length-1;i++){                         
									...
									html +='<div id="comment-pane" class="comment-pane">'
												...
												+'<label class="comment-user" style="color:sienna;font-size:12px;" >'+historyDict[i].remarks+'</label><br><font class="comment-date">'+historyDict[i]["remarksDate"]+' </font></div>'
												+'</div>'
											+'</div>';
				...
				if(name == "Despatch Documents to CO"){
					html +='<tr class="gradeA odd">'
							...
							+'<td width="15%" class=" ">'
								+'<div class="form-group text-center">'
								+'<div class="">'
									+'<input type="checkbox" id="Mem_'+App_Mem_ID+'_'+App_Loan_ID+'_'+loadData[i].id+'" class="checkBoxClass" style="width:20%;margin-left:-5px;"></input>&nbsp<a href="javascript:void(0)"  onclick="unClaimAlertDispatchDocuments('+"'"+loadData[i].id+"'"+')" class="btn btn-red btn-icon icon-left" style="height:30px;padding-left:30px;width:85px;">'
