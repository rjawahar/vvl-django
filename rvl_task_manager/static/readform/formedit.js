...
window.onload = function(){
	...
	if(screenHeight){
		...
		document.getElementById('right-pane-task').style.maxHeight = (slipHeight*2 + 80)+'px';
...
function submitForm(){
	...
	var formData_biz_address_fk_alf_node_name_1 = $('#biz_address_fk_alf_node_name_img_1').serialize();
	var formData_biz_address_fk_alf_node_name_2 = $('#biz_address_fk_alf_node_name_img_2').serialize();

	console.log('formData_biz_address_fk_alf_node_name_1');
	console.log(formData_biz_address_fk_alf_node_name_1);
	console.log(formData_biz_address_fk_alf_node_name_2);

	console.log("jQuery('#biz_address_fk_alf_node_name_img_1')[0]");
	console.log(jQuery('#biz_address_fk_alf_node_name_img_1'));
	console.log(jQuery('#biz_address_fk_alf_node_name_img_1')[0]);
	console.log(jQuery('#biz_address_fk_alf_node_name_img_1')[0].files);

	/*jQuery.each(jQuery('#biz_address_fk_alf_node_name___1')[0].files, function(i, file) {
	...
	for(var i=1; i <= data.mlcompositeArray[0]['memberAssetArray'].length; i++){
		...
		if(document.getElementById("fk_member_asset_type_id_"+i+"")){
			...
			if( self_occupation_id != ""){
				...
				//data.mlcompositeArray[0]['memberAssetArray'][i-1]["fk_member_asset_type_id"] = self_occupation_id;
			
			}
		}
	}

	//Transportation_communication_assets = data.mlcompositeArray[0]['memberAssetArray'];	
	...
	for(var i=1; i<=data.mlcompositeArray[0]['memberNetworkArray'].length;i++){
		...
		if(document.getElementById("network_details_question_"+i+"")){
			...
			if( (network_value  != "") && (network_question !="") ){
				...
				/*data.mlcompositeArray[0]['memberNetworkArray'][i-1]["network_details_question"]	= 1;
				data.mlcompositeArray[0]['memberNetworkArray'][i-1]["network_details_answer"]	= 1;*/

			}
		}
	}
	//network_details = data.mlcompositeArray[0]['memberNetworkArray'];
	...
	for(var i=1; i <= data.mlcompositeArray[0]['businessStaffArray'].length;i++){
		...
		if(document.getElementById("fk_biz_staff_rel_type_id_"+i+"")){
			...
			if( (staffVal  != "") && (jobVal !="") ){
				...
				//data.mlcompositeArray[0]['businessStaffArray'][i-1]["fk_biz_staff_rel_type_id"] = parseInt(staffVal);//1
				//data.mlcompositeArray[0]['businessStaffArray'][i-1]["biz_staff_work_hour"] = jobVal;//1
			}
		}
	}

	//staff_details = data.mlcompositeArray[0]['businessStaffArray'];
	...
	for(var i=1; i <= data.mlcompositeArray[0]['memberFamilyArray'].length;i++){
		...
		if(document.getElementById("fk_family_member_education_type_id_"+i+"")){
			...
			//data.mlcompositeArray[0]['memberFamilyArray'][i-1]["family_member_employment_type"]	= occupation;
			//data.mlcompositeArray[0]['memberFamilyArray'][i-1]["fk_family_member_education_type_id"]	= parseInt(education);
		}
	}
	//family_details = data.mlcompositeArray[0]['memberFamilyArray'];


	//console.log("Transportation_communication_assets");
	//console.log(Transportation_communication_assets);
