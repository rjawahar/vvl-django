...
function submitFormData(id){
	...
	if(window.parsedUserList && parsedUserList.length>0 || window.groups && groups != ""){
		if(window.parsedUserList && parsedUserList.length>0 ){
			...
			if(groups == "CrMgr"){
	    		...
	    		console.log("===============================");
	    		console.log("=======getProcessDataList======");
	    		var id = $("#loanFilterDropdown").val();
	      		[fromDate,toDate,userName] = validateInputFields(id);
				...
				if(status == "Rejected" || status == "Approved"){
					...
					getcompletedProcessList(fromDate,toDate,userName)
				}
				else{
...
function setDataOnLoad(){
	...
	//window.onload = function(){
		...
		var filterHTML = '<div style="width:80%;display:inline-flex;"><select class="extra select form-control" style="margin-bottom:6%; height:30px;width:12%; margin-right:2px;" id="regionMgr" onchange = "loadState()">'						
						+'</select>'
						+'<select class="extra select form-control" style="width:15%; margin-right:2px;height:30px;" id="State" onchange = "loadregion()">'
							+'<option value=""> Select State </option>'
						+'</select>'
						+'<select class="extra select form-control" style="width:16%; margin-right:2px;height:30px;" id="Region" onchange = "ClusOfficeLoad()">'
							+'<option value=""> Select Region </option>'
						+'</select>'
						+'<select class="extra select form-control" style="width:22%; margin-right:2px;height:30px;" id="clusterOffice" onchange = "loadCluster()">'
							+'<option value="0"> Select Cluster Office </option>'
						+'</select>'
						+'<select class="extra select form-control" style="width:23%; margin-right:2px;height:30px;" id="cluster" onchange = "userSelect()">'
							+'<option value=""> Select Cluster Center </option>'
						+'</select>'
						+'<select class="extra select form-control" style="width:19%; margin-right:2px;height:30px;" id="userSelect">'
							+'<option value=""> Select User </option>'
						+'</select><button type="submit" onclick="submitFormData('+filterId+');" class="btn btn-default btn-md pull-right" style="margin-left:5%;width:10%;font-size:13px;font-weight:bold;">Submit</button></div>';
...
function filterOnchange(){
	//$("#selectLocationFields").css("display","block");
				...
				if($.inArray(keyFields[i], selectionFields) != -1){
					...
					if(document.getElementById(keyFields[i])){
						if(data.mlcompositeArray[0][keyFields[i]] == 0)
							if (typeof(data.mlcompositeArray[0][keyFields[i]]) !== 'string'){
								data.mlcompositeArray[0][keyFields[i]] = "";
							} 
							console.log(data.mlcompositeArray[0][keyFields[i]]);
							console.log(typeof(data.mlcompositeArray[0][keyFields[i]]));
							//data.mlcompositeArray[0][keyFields[i]] = "";
						document.getElementById(keyFields[i]).value = data.mlcompositeArray[0][keyFields[i]];
					}
				}
...
function UploadDocsRework(){
	...
	var doc_url_count = $('.alf_node_ref_img').length;
  
	console.log('alf_node_ref_img  :::'+ doc_url_count);
...
function getDataListWithFilter(){
	...
	console.log("getDataListWithFilter-------");
	/*if(!$('#loanFilterDropdown').val()){
        ...
        var theImg = '<div class="loading"><img style="width:350px;" src="/static/images/buffer-loading.gif">' + '<div style="padding-top:1%;" ><label style="padding-top:1%;">LOADING ...</label>' + '</div>' + '</div>';
	$(".popup").empty().append(theImg).fadeIn();
	console.log("Entering filtered list");
        $.ajax({
        url : '/filteredList/',
        type: 'POST',        
        datatType: 'json',            
