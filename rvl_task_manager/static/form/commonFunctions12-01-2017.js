...
function submitProcessFormResolveQuery(status, val) {
	[currentDateValue,currentTime] = currentDate();
	console.log("32222222222222222222222222");
	fileUploadRework();
	var processupdate = {
		'variables': {
			'Cibil_Verification_Status': {
...
function submitFormData(id){
	...
	if(document.getElementById("locationFilterDropdown")){
		if ($('#locationFilterDropdown').val()) {
	      getDataListWithFilter();     
	    	...
	    	//validateInputFields(id);
	  		var data = validateInputFields(id);
	  		if(data == false){
	  			return false;
	  		}
	  		else{
