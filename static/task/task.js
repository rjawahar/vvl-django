...
function leftPaneTaskList_Load(){
	...
	//document.getElementById('left-pane-my-loan').innerHTML = '</label>View My Loans</label>';
				...
				if(taskData.grp == "Distributor"){
					...
					html +='<tr class="gradeA odd">'
							...
							+'<td width="15%" class=" ">'
							    +'<div class="form-group text-center">'
								+'<div class="" style="float:left;">'
									+'<a href="javascript:void(0)" onclick="claimconfirmBox('+"'"+loadData[i].id+"'"+')" class="btn btn-red btn-icon icon-left" style="height:30px;padding-left:40px;">'/*padding:4px 4px;*/
									+'<i class="icon-entypo-check" style="width:30px;height: 28px;color: white;padding:6px;"></i>'
									+'Claim</a>'
								+'</a>'
								+'&nbsp&nbsp&nbsp&nbsp&nbsp<label class ="history" onclick="getHistory('+"'"+loadData[i].processInstanceId+"','"+App_Mem_ID+"','"+App_Mem_Name+"'"+');" ><i class="entypo-back-in-time" style="font-size:medium;cursor:pointer;"></i></label>'
								+'</div>'
							    +'</div>'
							+'</td>'	
						+'</tr>';
...
function getHistory(processId,memberid,memberName){
	...
	$.ajax({
		...
		for( var data in history){
			var arr={};
			if(history[data]["activityType"] == "serviceTask" || history[data]["activityType"] == "userTask" ){
				    ...
				    $.ajax({
					    ...
					    success: function (data2) {
							...
							if(data2["mlValidationArray"]){
								...
								$.alert("As of now, no comments for the process!");					
							}							
						}
			  	    });	
			    }
			    else{
			    	$.alert("Too many requests to Server! Please try again after some time.");		
			    }
		     }
		 });
	   }
				...
				if(name == "Despatch Documents to CO"){
					html +='<tr class="gradeA odd">'
							...
							+'<td width="15%" class=" ">'
								+'<div class="form-group text-center">'
								+'<div class="">'
									+'<input type="checkbox" id="Mem_'+App_Mem_ID+'_'+App_Loan_ID+'_'+loadData[i].id+'" class="checkBoxClass" style="width:20%;margin-left:-5px;"></input>&nbsp<a href="javascript:void(0)" onclick="unClaimconfirmBox('+"'"+loadData[i].id+"'"+');" class="btn btn-red btn-icon icon-left" style="height:30px;padding-left:30px;width:85px;">'
									+'<i class="icon-entypo-cancel" style="width:25px;height: 28px;color: white;padding:6px;"></i>'
									+'Unclaim</a>'
								+'</a>'
								+'&nbsp&nbsp<label class="history" onclick="getHistory('+"'"+loadData[i].processInstanceId+"','"+App_Mem_ID+"'"+');"><i class="entypo-back-in-time" style="font-size:medium;cursor:pointer;"></i></label>'
								+'</div>'
								+'</div>'
							+'</td>'	
						+'</tr>';
					...
					html +='<tr class="gradeA odd">'
							...
							+'<td width="15%" class=" ">'
								+'<div class="form-group text-center">'
								+'<div class="" style="float:left;">'
									+'<a href="javascript:void(0)" onclick="unClaimconfirmBox('+"'"+loadData[i].id+"'"+');" class="btn btn-red btn-icon icon-left" style="height:30px;padding-left:40px;">'
									+'<i class="icon-entypo-cancel" style="width:30px;height: 28px;color: white;padding:6px;"></i>'
									+'Unclaim</a>'
								+'</a>'
								+'&nbsp&nbsp&nbsp&nbsp&nbsp<label class="history" onclick="getHistory('+"'"+loadData[i].processInstanceId+"','"+App_Mem_ID+"','"+App_Mem_Name+"'"+');"><i class="entypo-back-in-time" style="font-size:medium;cursor:pointer;"></i></label>'
								+'<strong style="cursor:pointer;color:#981b1b;" onclick="getMemberId('+"'"+loadData[i].processInstanceId+"','"+loadData[i].processInstanceId+"','"+loadData[i].id+"','memberDetails'"+')"><i class="entypo-user"></i></strong>'
