...
function setURL(id){
		...
		$("#"+id).change(function(){
			document.getElementById("family_member_with_residence_photo"+id_select).value = this.value;
			readURL(this, id+"_img");
		});
}
...
function submitResidentForm(){
	...
	if(document.getElementById("residence_photo_1").files.length == 0){
		$("#family_member_with_residence_photo1").css('background-color','yellow');
		$("#family_member_with_residence_photo1").css('color','black');
		validation =1;
		label = document.getElementById("family_member_with_residence_photo1_label").innerHTML;
	...
	jQuery.each(jQuery('#residence_photo_1')[0].files, function(i, file) {
